...
14. موجودی `پد الکلی` در نرم‌افزار محک و بازارا `۵۲۰۰` و قیمت آن `۳۶۰۰۰` ریال تأیید شد؛ نگاشت Mixin برابر `۳۶۰۰` تومان است.
15. اجرای تک‌محصولی مستقل از sync عمومی، با مجوز موقت و rollback کنترل‌شده انجام می‌شود.
16. guard نوشتن پس از انتقال بسته شد و تلاش rollback در حالت بسته بدون تغییر خارجی رد شد.
17. قیمت و موجودی کدهای `453`، `454` و `872` در رابط محک با مقادیر API تطبیق و تأیید شدند.
18. alt تصویر از نام فعلی محصول گرفته می‌شود، نه Title تصویر؛ چون عنوان برخی تصاویر قدیمی یا نامعتبر است.
...
## نتیجه انتقال اولین کالای واقعی

در `2026-09-20` فرمان محدود تک‌محصولی، پد الکلی با `ProductDetailId=11629315` را به Mixin منتقل کرد:

- Mixin product ID: `223`
- قیمت: `۳۶۰۰` تومان، متناظر با `۳۶۰۰۰` ریال محک
- موجودی: `۵۲۰۰`، مطابق نرم‌افزار محک و فهرست بازارا
- وضعیت: فعال و قابل نمایش قیمت
