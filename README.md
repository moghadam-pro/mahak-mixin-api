...
- انتقال کنترل‌شده چهار کالای واقعی همراه نام، قیمت، موجودی، دسته‌بندی و تصویر با موفقیت آزمایش شد؛ این رکوردهای آزمایشی سپس از فروشگاه پاک شدند.
- تبدیل قیمت حساب محک از ریال به تومان با `SYNC_PRICE_DIVISOR=10` انجام می‌شود.
- baseline فقط‌خواندنی، دقیقاً `۱۸۲۸` کالا، `۱۸۲۸` جزئیات کالا، `۱۸۲۸` رکورد موجودی، `۹۳۲` تصویر و `۹۳۴` گالری را بدون نوشتن در میکسین ثبت کرده است.
- همگام‌سازی افزایشی دائمی با Cron یک‌دقیقه‌ای فعال است و کالاهای جدید یا تغییرکرده پس از baseline را پردازش می‌کند.
- اجرای تکراری از ساخت محصول و تصویر تکراری جلوگیری می‌کند.
- ۱۴ تست خودکار روی سرور اصلی بدون خطا اجرا شده‌اند.
- داشبورد فارسی فعال است و از فونت محلی وزیرمتن استفاده می‌کند.
- سرویس عملیاتی با `SYNC_DRY_RUN=false` اجرا می‌شود؛ عملیات پرریسک مانند ورود کامل، انتقال تک‌محصولی و پاک‌سازی، گارد مستقل و موقت دارند.
...
3. Bridge در اجرای زمان‌بندی‌شده یک‌دقیقه‌ای، تغییرات جدید را از محک دریافت می‌کند.
...
نکته: API محک تغییرات را با `RowVersion` تحویل می‌دهد. ارسال دوباره کالایی که پیش از baseline ارسال شده و در محک هیچ تغییری نکرده است، الزاماً RowVersion تازه تولید نمی‌کند؛ چنین کالایی باید یک‌بار با فرمان تک‌محصولی وارد شود. کالاهای جدید یا ویرایش‌شده بعد از baseline به‌صورت خودکار در اجرای بعدی Cron پردازش می‌شوند.
...
| قیمت فروش | `ProductDetail.Price2` و در صورت صفر/خالی بودن `ProductDetail.Price1` | `price` | فعال، با تبدیل ریال به تومان؛ اگر هر دو صفر باشند قیمت صفر ارسال می‌شود |
...
baseline باید فقط یک‌بار و پیش از فعال‌شدن worker اجرا شود. اجرای دوباره آن می‌تواند checkpointها را تا وضعیت فعلی جلو ببرد و تغییرات منتظر انتقال را به‌عنوان وضعیت پایه ثبت کند.

### کالای ارسال‌شده پیش از baseline

اگر کالا پیش از baseline در بازارا ارسال شده و ارسال مجدد آن RowVersion تازه ایجاد نمی‌کند، ابتدا شناسه `ProductDetail` آن را با فرمان تشخیصی پیدا کنید و سپس انتقال کنترل‌شده را اجرا کنید:

```bash
SYNC_PAGE_SIZE=5000 php8.3 bin/console mahak:products:candidates > var/log/mahak-candidates.json
php8.3 bin/console sync:product:preview PRODUCT_DETAIL_ID FALLBACK_CATEGORY_ID
php8.3 bin/console sync:product:apply PRODUCT_DETAIL_ID FALLBACK_CATEGORY_ID
php8.3 bin/console sync:product:image:apply PRODUCT_DETAIL_ID
```

برای این عملیات فقط `SYNC_ALLOW_SINGLE_PRODUCT_WRITE=true` را موقتاً فعال کنید. مسیر تک‌محصولی از نسخه `0.3.1` همان قواعد معتبر کاتالوگ را برای وضعیت حذف، موجودی و دسترس‌پذیری اعمال می‌کند. پس از پایان، گارد را دوباره `false` کنید.
...
فرمان زیر شناسه همه محصولات واقعاً موجود در Mixin را صفحه‌بندی می‌کند و در batchهای ۱۰۰تایی حذف می‌کند؛ بنابراین حذف دستی تعدادی از محصولات باعث اختلاف یا خطا نمی‌شود. دسته‌ها و تاریخچه اجرا نگه داشته می‌شوند. پس از موفقیت همه batchها، mapping، snapshot و checkpointهای محصول نیز تراکنشی پاک می‌شوند تا ورود بعدی از صفر شروع شود. فرمان هنگام اجرای sync فعال، به‌دلیل lock مشترک، از حذف خودداری می‌کند.
